...
public class StreamTest {
	public static void main(String[] args) {
		// forEach()
		// 여러 값을 가지고 있는 컬렉션 객체에서 forEach 메소드를 사용할 수 없다.
		// forEach 메소드에는 consumer 인터페이스 타입의 값을 전달해야 한다.
		// Consumer는 함수형 인터페이스이기 때문에 람다식을 사용 할 수 있다.
		// 매개변수에는 컬렉션 객체 안에 들어았는 값들이 순서대로 담기고,
...
//		ArrayList<Integer> datas = new ArrayList<Integer>();
		...
		// values는 컬렉션 타입, forEach()를 쓰기 위해 .stream() 붙여 stream 타입으로 변형 
//		chinaTown.values().stream().forEach(price -> System.out.println(price));
		// collection 타입도 forEach를 많이 쓰기 때문에 stream()을 붙이지 않고 사용 가능
...
//		datas.forEach(value -> System.out.println(value));
		...
		// 소속::메소드명 --> 전달받은 값을 메소드의 매개변수로 바로 전달해 준다.
