...
//		String alpha = "ABCDEF";
//		alpha.chars().filter(n -> n != 68).forEach(number -> System.out.println((char)number));
...
//		datas.stream().filter(fruit -> fruit.charAt(0) >= 'A').filter(fruit -> fruit.charAt(0) <= 'Z').forEach(null);
